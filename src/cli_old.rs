...
use clap::Parser;
use letta::types::agent::{AgentType, CreateAgentRequest, EmbeddingConfig, ListAgentsParams};
...
use letta::types::source::{CreateSourceRequest, ListFilesParams, ListPassagesParams};
...
async fn main() -> miette::Result<()> {
    ...
    match args.command {
        ...
        Command::Sources(sources_cmd) => match sources_cmd {
            SourcesCommand::List { output } => {
                list_sources(&client, &output).await?;
            }
            ...
            SourcesCommand::Get { id, output } => {
                get_source(&client, &id, &output).await?;
            }
            SourcesCommand::Delete { id, yes } => {
                delete_source(&client, &id, yes).await?;
            }
            SourcesCommand::Files(files_cmd) => match files_cmd {
                ...
                FilesCommand::Get {
                    ...
                    get_file(&client, &source_id, &file_id, content, &output).await?;
                }
                FilesCommand::Delete {
                    source_id,
                    file_id,
